...
#lcp for light response curves for seliganella

lrc <- read.csv("raw_data/selaginella_lightcurve.csv")
  lrc$id <- paste(lrc$species, lrc$sample, sep="-")

#function creates new dataframe with just Par and Photo 
...
#inspects all lrc plots-------------
  ...
  l_ply(curves2_ls,  function(x){ par(mar=c(5,5,1,1)) 
    plot(x$PARlrc,x$photolrc, ylim=c(-1,8),cex=1.5,xlim=c(0,825),
...
#now we have a list of light response curves to send to lrc function....
test <- curves2_ls[[1]]

testfits <- lrcnls_func(curves2_ls[[2]])
